...
var defaults = {
  ...
  verbose: false,
  simulate: false,
  // only parse and return the data, no file creation
  parse: false,
  colorize: true,
  markdown: true,
  types: false,
...
function createApidocSwagger(options) {
  ...
  try {
    ...
    if (app.options.parse !== true) {
      ...
      api["swaggerData"] = JSON.stringify(apidocSwagger.toSwagger(
        apidocData, projectData, app.options.types, app.options.ignoreGroups
      ), null, '  ');
