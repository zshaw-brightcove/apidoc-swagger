...
function createSchema(fields, definitions, defName, objName, isSuccess){
	...
	if (fieldType.toLowerCase().indexOf('object') >= 0 || fieldType.toLowerCase() == 'array' || isSuccess){
		...
		if ((fieldArrayResult.topLevelRefType.toLowerCase() == 'object') || isSuccess) {
...
function createSuccessResults(verbs, definitions){
	return _.mapValues(verbs.success ? verbs.success.fields : [], function(success){
		...
		if (success.length > 0  && success[0].field && success[0].field != 'null'
			...
			result.schema = createSchema(success, definitions, verbs.name + "Result", verbs.name + "Result", true);
		}
		return result;
	});
}
